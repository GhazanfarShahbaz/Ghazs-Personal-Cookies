...
def process_delete_file(delete_file_form) -> Dict[str, str]:
    ...
    This function takes a dictionary `delete_file_form` representing the delete parameters for a file.
    The function then calls the `delete_file()` method with these parameters to delete the file from a storage system.

    Args:
        delete_file_form: A dictionary containing the bucket and file path of the file to be deleted.
...
def delete_process_list_bucket_files(list_file_form) -> Dict[Dict, List[str]]:
    ...
    This function takes a string `bucket` and an optional string `prefix` and returns a dictionary containing the files in the
    specified bucket. If a prefix is provided, only files matching the prefix will be included.
