...
def process_create_syllabus(syllabus_data: dict) -> None:
    ...
    This function takes a dictionary `syllabus_data` representing the syllabus data from a request and creates a new syllabus in
...
def process_update_syllabus(update_form: dict) -> None:
    ...
    This function takes a dictionary `update_form` representing the update parameters for a syllabus.
    The function then calls the `SyllabusRepository().update()` method with these parameters to update the syllabus in the
    syllabus repository.
...
def process_get_syllabus_request(filter_form: dict) -> List[dict]:
    ...
    This function takes a dictionary `filter_form` representing the filter parameters for a syllabus request,
    and calls the `SyllabusRepository().get()` method with these parameters to retrieve a list of syllabuses that
    match the filter. The returned syllabuses are then converted to a list of dictionaries using the `syllabus_type_list_to_event_dict_list()`
    function.
    ...
    Returns:
        A list of dictionaries representing the retrieved syllabuses. Each dictionary contains the fields "SyllabusId", "Name",
        "CourseCode", "CourseTitle", "Instructor", and "Syllabus".
    """
...
def process_delete_syllabus_request(delete_form: dict) -> None:
    ...
    This function takes a dictionary `delete_form` representing the delete parameters for a syllabus request.
    The function then calls the `SyllabusRepository().delete()` method with these parameters to delete the given syllabus from
    the syllabus repository.

    Args:
        delete_form: A dictionary containing the syllabus ID or course code that needs to be deleted.
