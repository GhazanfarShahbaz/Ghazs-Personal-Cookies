...
def get_divinfo():
    ...
    ReccomendedBook = data[
        (abs(data["Average_Rating"] - rating) <= 0.5)
...
def getMovieReccomendation():
    ...
    bestRecc = movies_data[
        (movies_data["genres"] == this_movie_data.genres.values[0])
    ...
    moviesInfo = {"movie_name": movie_names, "movie_id": movies_imdb}
...
def gameSearch():
    ...
    responseDict = requests.request(
        "GET",
        f"https://rawg-video-games-database.p.rapidapi.com/games?search={request.args.get('search')}",
    ...
    for data in responseDict["results"]:
        specific_information = requests.request(
            "GET",
            f"https://rawg-video-games-database.p.rapidapi.com/games/{data['slug']}",
            headers=game_headers,
        ).json()
...
def gameRecc():
    ...
    find_match = games_data[
        (games_data["Genre"] == game_data.Genre.values[0])
        & (games_data["Rating"] == game_data.Rating.values[0])
        & (
            abs(games_data["Year_of_Release"] - game_data["Year_of_Release"].values[0])
            <= 5.0
        )
    ...
    find_match.drop(
        find_match[find_match["Name"] == game_data.Name.values[0]].index, inplace=True
    )
    ...
    for x in range(3):
        games_list = requests.request(
            "GET",
            f"https://rawg-video-games-database.p.rapidapi.com/games?search={find_match.iloc[[x]]['Name'].values[0]}",
            headers=game_headers,
        ).json()
...
def add_data_to_csv():
    ...
    if request.args.get("book_name") in existingData.Book_Name.values:
        ...
        bookData = DataFrame(
            {
                ...
                "Ratings_Count": request.args.get("ratings_count"),
        ...
        bookData.to_csv(
            "static/csv/googleBooksData.csv", mode="a", header=False, index=False
        )
