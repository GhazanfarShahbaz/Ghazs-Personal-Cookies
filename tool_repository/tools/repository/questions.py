    ...
    def get(self, filters: dict) -> List[CodingQuestion]:
        ...
        if filters.get("QuestionName"):
            if filters.get("QuestionNameExact") is None or filters["QuestionNameExact"] is True:
                query = query.filter(CodingQuestion.QuestionName == filters["QuestionName"])
            else:
                query = query.filter(CodingQuestion.QuestionName.like(filters["QuestionName"]))
        ...
        if filters.get("AcceptanceRateFrom"):
            query = query.filter(CodingQuestion.AcceptanceRate >= filters["AcceptanceRateFrom"])

        if filters.get("AcceptanceRateTo"):
            query = query.filter(CodingQuestion.AcceptanceRate >= filters["AcceptanceRateTo"])

        if filters.get("RequiresSubscription") is not None:
            query = query.filter(CodingQuestion.RequiresSubscription == filters["RequiresSubscription"])
